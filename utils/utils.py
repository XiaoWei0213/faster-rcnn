...
def bbox2loc(src_bbox, dst_bbox):
    ...
    dh = np.load(base_height / height)

    loc = np.vstack((dx, dy, dw, dh)).transpose() #vstack垂直堆叠 转置
    return loc
...
def nms(detections_class, nms_thres=0.7):
    ...
    while np.shape(detections_class)[0]:
        ...
        # np.expand_dims扩充数组的维度，axis表示添加维度的位置，如(2,3) -> (1,2,3) axis=0
...
class DecodeBox():
    def __init__(self,std,mean,num_classes):
        self.std = std
        self.mean = mean
        self.num_classes = num_classes + 1 #背景加1
    ...
    def forward(self, roi_cls_locs, roi_scores, rois, height, width, nms_iou, score_thresh):
        ...
        roi_cls_loc = roi_cls_loc.view([-1, self.num_classes, 4]) #view函数
...
class ProposalTargetCreator(object):
    ...
    def __call__(self, roi, bbox, label,
        ...
        #正负样本的平衡，满足建议框和真实框重合度小于neg_iou_thresh_hi大于neg_iou_thresh_lo作为负样本
